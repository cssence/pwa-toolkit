(function (window, document, navigator) {
	...
	var update = function () {
		...
		fetch(url).then(function (response) {
			...
			$(".contacts").addEventListener("click", function (event) {
				...
				//window.scrollTo(0, 0);
				$(".action--close").addEventListener("click", function (event) {
					$(".details").remove();
					document.body.classList.toggle("show-details");
				});
			});
	...
	$(".action--modify-source").addEventListener("click", function (event) {
		$("header").classList.toggle("show-source", true);
		var input = $(".source-url");
		input.value = window.localStorage.getItem(URL_KEY);
		input.select();
	});
	$(".action--back").addEventListener("click", function (event) {
		$("header").classList.toggle("show-source", false);
	});
	$(".source-url").addEventListener("keypress", function (event) {
		if (event.keyCode === 13) {
			$(".action--update").click();
		}
	});
	$(".action--update").addEventListener("click", function (event) {
		window.localStorage.setItem(URL_KEY, $(".source-url").value);
		$("header").classList.toggle("show-source", false);
		update();
	});
	update();
